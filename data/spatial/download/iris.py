...
def execute(context):
    ...
    with context.progress(label = "Downloading CONTOURS-IRIS_3-0__SHP__FRA_2023-01-01.7z ...", minimum_interval=5) as progress:
        def progress_hook(count, block_size, total_size):
            percent = int(count * block_size * 100 / total_size)
            progress.tracker.total = total_size
            progress.update(block_size)

        zip_file, headers = urllib.request.urlretrieve(url, "%s/CONTOURS-IRIS_3-0__SHP__FRA_2023-01-01.7z" % context.path(), reporthook=progress_hook)
