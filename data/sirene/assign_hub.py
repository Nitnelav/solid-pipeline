...
def execute(context):
    ...
    gdf_sirene["hub_id"] = gdf_sirene.apply(lambda x: gdf_hubs.distance(x.geometry).idxmin(), axis=1)
    gdf_sirene["hub_distance"] = gdf_sirene.apply(lambda x: x.geometry.distance(gdf_hubs.loc[x["hub_id"]].geometry), axis=1)
