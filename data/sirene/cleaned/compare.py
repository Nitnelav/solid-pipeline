...
def execute(context):
    ...
    employees_eqasim = df_cleaned_eqasim["employees"].value_counts(
        normalize=True, bins=employees_bins, sort=False
    )
    employees_eqasim.name = "eqasim (nb: %d)" % len(df_cleaned_eqasim)

    employees_horizon = df_cleaned_horizon["employees"].value_counts(
        normalize=True, bins=employees_bins, sort=False
    )
    employees_horizon.name = "horizon (nb: %d)" % len(df_cleaned_horizon)

    employees_adrien = df_cleaned_adrien["employees"].value_counts(
        normalize=True, bins=employees_bins, sort=False
    )
    employees_adrien.name = "adrien (nb: %d)" % len(df_cleaned_adrien)

    df_employees = pd.concat(
        [employees_eqasim, employees_horizon, employees_adrien], axis=1
    )
    print(df_employees)
    ...
    plt.tight_layout()
    plt.savefig(
        "%s/sirene_compare_employees.png"
        % context.config("sirene_compare_method_output")
    )
    ...
    plt.tight_layout()
    plt.savefig(
        "%s/sirene_compare_st8.png" % context.config("sirene_compare_method_output")
    )
    ...
    plt.tight_layout()
    plt.savefig(
        "%s/sirene_compare_st45.png" % context.config("sirene_compare_method_output")
    )
    ...
    plt.tight_layout()
    plt.savefig(
        "%s/sirene_compare_st20.png" % context.config("sirene_compare_method_output")
    )
